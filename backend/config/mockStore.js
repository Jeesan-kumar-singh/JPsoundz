...
// Adjectives and nouns lists to construct 100 unique song titles
...
// Unsplash premium dark-themed imagery
const covers = [
...
// Helper to deterministically generate 100 tracks
const generateMockSongs = (artistsList, count = 100) => {
  const songs = [];
  for (let i = 1; i <= count; i++) {
    const artist = artistsList[i % artistsList.length];
    
    // Use Math.sin with prime multiplier for deterministic pseudo-random array indexing
    ...
    // Alternate soundhelix files (1 to 16) so playing songs operates correctly
...
export const mockStore = {
  users: mockUsers,
  songs: generateMockSongs(mockUsers.filter(u => u.role === 'artist'), 100),
  collabPosts: [
    {
      _id: 'mock_collab_1',
      author: {
...
// Hash passwords in mockStore on startup
const salt = bcrypt.genSaltSync(10);
const hashedPass = bcrypt.hashSync('password123', salt);
mockStore.users.forEach(u => u.password = hashedPass);
